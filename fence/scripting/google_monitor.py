...
def validation_check(db):
    ...
    for google_project_id, sa_emails in list(project_service_account_mapping.items()):
...
def _send_emails_informing_service_account_removal(
    ...
    for email, removal_reasons in list(invalid_service_account_reasons.items()):
        if removal_reasons:
    ...
    if general_project_errors or non_reg_sa_errors or access_errors:
        ...
        if access_errors:
            for project, removal_reasons in list(access_errors.items()):
                for reason in removal_reasons:
                    content += "\n\t\t - {}".format(reason)

        if non_reg_sa_errors:
            for sa_email, removal_reasons in list(non_reg_sa_errors.items()):
...
def email_users_without_access(
    ...
    if len(users_without_access) == len(user_emails):
        ...
        for user, projects in list(users_without_access.items()):
            logger.info(
